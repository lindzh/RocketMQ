...
public class DeleteTimedKvConfigCommandTest extends BasicTimedKVTest{

    private static DeleteTimedKvConfigCommand deleteTimedKvConfigCommand = Mockito.spy(DeleteTimedKvConfigCommand.class);
    ...
    public void init() throws NoSuchFieldException, IllegalAccessException, RemotingException, MQClientException, InterruptedException, SubCommandException {
        ...
        init4TimedKV(deleteTimedKvConfigCommand,mQClientAPIImpl);
        ...
        timedConfigHashMap.get(TIMED_NAMESPACE).put("del2",new TimedConfig("value2",System.currentTimeMillis()));
    ...
    public void testExec() throws SubCommandException {
        CommandLine commandLine = parseCommandLine(deleteTimedKvConfigCommand, new String[]{"-s","namespace1","-k","del1"});
        deleteTimedKvConfigCommand.execute(commandLine,null,null);
        String printContent = getPrintContent();
        System.err.println(printContent);
        Assert.assertTrue(printContent.contains("success"));
        Assert.assertTrue(timedConfigHashMap.get(TIMED_NAMESPACE).size()==2);
    }
}
